...
Lets you:
  - Edit the geometry/physics parameters (including the still-unverified inlet_normal
    and outlet_area_raw)
...
if running:
    ...
    if st.sidebar.button("Launch training", type="primary"):
        ...
        launch_training(st.session_state["hydra"])
        st.sidebar.success("Training launched.")
        time.sleep(1)
        st.rerun()
...
with tab_params:
    ...
    st.caption("These two are still flagged from your geometry summary — confirm against your STL/CAD before launching.")
    c1, c2 = st.columns(2)
    with c1:
    ...
    with c2:
        P["outlet_area_raw"] = st.number_input(
            "outlet_area_raw ⚠️ (raw mesh units, pre-scale)",
            value=float(P["outlet_area_raw"]), format="%.6g", key="outlet_area_raw",
        )
        st.caption("Used as: outlet_area = outlet_area_raw × scale²")

    st.divider()
    st.subheader("Core physics & geometry")
    ...
    st.divider()
    st.subheader("Fluid properties & weighting")
    c1, c2 = st.columns(2)
    with c1:
