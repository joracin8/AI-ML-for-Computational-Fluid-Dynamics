...
def launch_training(hydra_overrides=None):
    ...
    try:
        ...
        # Pin hydra.run.dir explicitly. Without this, PhysicsNeMo's default config
        # folds the CLI overrides above (e.g. "arch.fully_connected.layer_size=256")
        # into the output folder name via hydra.job.override_dirname. That string
        # contains "=" and "." characters that Windows' filesystem APIs choke on
        # (WinError 3: "The system cannot find the path specified"). Forcing a clean
        # date/time-based path here sidesteps that entirely, on any OS.
        cmd.append("hydra.run.dir=./outputs/${now:%Y-%m-%d}/${now:%H-%M-%S}")
...
with tab_training:
    ...
    with st.expander("Preview command-line overrides"):
        ...
        preview_cmd += ' hydra.run.dir=./outputs/${now:%Y-%m-%d}/${now:%H-%M-%S}'
        st.code(preview_cmd, language="bash")
        st.caption("The hydra.run.dir override is added automatically at launch — pins the output folder to a clean date/time path so Windows doesn't choke on '=' characters from the overrides above.")
