...
"""
Aneurysm training script — parameterized version of the PhysicsNeMo aneurysm example.

This script is identical in physics/structure to the original NVIDIA example, except
that the geometry/physics parameters (the ones tracked in the dashboard's parameter
table) are loaded at runtime from `run_params.json`, written by the Streamlit dashboard,
rather than being hardcoded. If `run_params.json` is not found next to this script, it
falls back to the original hardcoded values so the script still runs standalone.

Network architecture and training-loop settings (layer_size, nr_layers, max_steps,
batch sizes, decay rate) are NOT read from run_params.json — they live in Hydra's
cfg object (conf/config.yaml) and are overridden via command-line arguments, e.g.:

    python aneurysm_train.py arch.fully_connected.layer_size=512 training.max_steps=500000

The dashboard builds this command line for you; you never need to type it by hand.
"""
...
# ---------------------------------------------------------------------------
# Default parameters (= original NVIDIA example values). Used only if
# run_params.json is missing, so the script still works standalone.
# ---------------------------------------------------------------------------
DEFAULT_PARAMS = {
    "nu": 0.025,
    "inlet_vel": 1.5,
    ...
    "inlet_area_raw": 21.1284,
    "inlet_center": [-4.24298030045776, 4.082857101816247, -4.637790193399717],
    "outlet_area_raw": 12.0773,
    "integral_continuity_1_value": 2.540,
    "integral_continuity_2_value": -2.540,
...
def load_params():
    """Load run parameters written by the Streamlit dashboard, falling back to
    the original hardcoded example values if no override file is present."""
    params_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "run_params.json")
    params = dict(DEFAULT_PARAMS)
    if os.path.exists(params_path):
        with open(params_path, "r") as f:
            overrides = json.load(f)
        params.update(overrides)
        print(f"[aneurysm_train] Loaded parameter overrides from {params_path}")
    else:
        print("[aneurysm_train] No run_params.json found, using built-in defaults")
    return params
...
def run(cfg: PhysicsNeMoConfig) -> None:
    ...
    point_path = to_absolute_path(P["stl_dir"])
    ...
    # geom params
    inlet_normal = tuple(P["inlet_normal"])
    inlet_area = P["inlet_area_raw"] * (scale**2)
    inlet_center = tuple(P["inlet_center"])
    inlet_radius = np.sqrt(inlet_area / np.pi)
    outlet_area = P["outlet_area_raw"] * (scale**2)
    ...
    ns = NavierStokes(nu=nu * scale, rho=P["rho"], dim=3, time=P["time_dependent"])
    ...
    integral_continuity = IntegralBoundaryConstraint(
        nodes=nodes,
        geometry=outlet_mesh,
        outvar={"normal_dot_vel": P["integral_continuity_1_value"]},
        batch_size=1,
        integral_batch_size=cfg.batch_size.integral_continuity,
        lambda_weighting={"normal_dot_vel": P["lambda_weighting_1"]},
    )
    ...
    integral_continuity = IntegralBoundaryConstraint(
        ...
        outvar={"normal_dot_vel": P["integral_continuity_2_value"]},
        batch_size=1,
        integral_batch_size=cfg.batch_size.integral_continuity,
        lambda_weighting={"normal_dot_vel": P["lambda_weighting_2"]},
    )
    domain.add_constraint(integral_continuity, "integral_continuity_2")

    # add validation data
    file_path = P["openfoam_csv"]
