...
RUN_NAME = os.path.splitext(os.path.basename(TRAIN_SCRIPT))[0]  # "aneurysm_train"
...
OUTPUTS_DIR = os.path.join(APP_DIR, "outputs", RUN_NAME)  # outputs/aneurysm_train/<date>/<time>/
...
def launch_training(hydra_overrides=None):
    ...
    try:
        ...
        # Pin hydra.run.dir to a literal, pre-computed path rather than passing the
        # "${now:%Y-%m-%d}" resolver syntax through argv. That string contains raw
        # "%" characters which can get mangled crossing the Windows command-line
        # boundary (cmd.exe/conda activation scripts treat "%" specially), causing
        # Hydra to silently fall back to a job-name-based folder (e.g. a folder just
        # called "aneurysm_train") instead of the date/time path. Computing the
        # timestamp here in Python and writing it as a literal string sidesteps
        # that entirely — no "%" ever crosses the process boundary.
        #
        # This must stay in sync with OUTPUTS_DIR above (both use RUN_NAME) so the
        # dashboard's Live Monitor / Results tabs look in the same place training
        # actually writes to.
        now = datetime.now()
        run_dir = os.path.join(
            "outputs", RUN_NAME, now.strftime("%Y-%m-%d"), now.strftime("%H-%M-%S")
        ).replace("\\", "/")
...
def find_latest_run_dir():
    """Hydra writes to outputs/<RUN_NAME>/<date>/<time>/ relative to the script's cwd."""
...
st.sidebar.caption(
    ...
    f"`outputs/{RUN_NAME}/<date>/<time>/` next to the script."
)
...
with tab_training:
    ...
    with st.expander("Preview command-line overrides"):
        example_now = datetime.now()
        example_run_dir = f"./outputs/{RUN_NAME}/{example_now.strftime('%Y-%m-%d')}/{example_now.strftime('%H-%M-%S')}"
        preview_cmd = " ".join(f"{k}={v}" for k, v in H.items())
        preview_cmd += f" hydra.run.dir={example_run_dir}"
        st.code(preview_cmd, language="bash")
        st.caption(
            "hydra.run.dir is computed in Python at launch time (not via Hydra's ${now:...} "
            "resolver) and added automatically — pins the output folder to a clean, literal "
            "date/time path so neither '=' from the overrides above nor '%' from a resolver "
            "string ever has to survive crossing the Windows command line."
        )
